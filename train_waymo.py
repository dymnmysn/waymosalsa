...
from printiou import print_miou_waymo as print_miou_results
from lovasz import Lovasz_softmax
from mappings import waymovalidfreqs
...
if __name__=='__main__':
    ...
    ff = FastFill(tofill=-1, indices=[0,1,2,3,4])
    ...
    train_dataset = SegmentationDataset(root = '/ari/users/ibaskaya/projeler/kittisalsa/data/waymo', 
                                    split = 'training', transform=transform_train, 
                                    pretransform=pretransform, fastfill=ff, iswaymo=True, width=2650)
    ...
    validation_dataset = SegmentationDataset(root = '/ari/users/ibaskaya/projeler/kittisalsa/data/waymo', 
                                        split = 'validation', transform=transform_valid, 
                                        pretransform=None, fastfill=ff, iswaymo=True, width=2650)
    validation_dataloader = torch.utils.data.DataLoader(validation_dataset, batch_size=batch_number, shuffle=False, drop_last=False, num_workers=10)

    bt = next(iter(train_dataloader))
    bv = next(iter(validation_dataloader))
    print("Shapes: ",bt[0].shape, bt[1].shape, bv[0].shape, bv[1].shape)
    ...
    for epoch in range(max_epochs):
        ...
        with torch.no_grad():
            for i, (images, masks) in tqdm(enumerate(validation_dataloader)):
                ...
                cwiou = calculate_classwise_intersection_union(preds, masks,num_classes=num_classes)
                batch_results.append(cwiou)

                miou_total += miou

        # Calculate and display mIoU metrics
        classwise_iou, mean_iou, total_iou = calculate_final_miou_from_batches(batch_results, num_classes=num_classes)
        ...
        if mean_iou > best_mean_iou:
            ...
            torch.save(model.state_dict(), f'waymo_best_model_state_dict.pth')
            print(f"New best mIoU: {best_mean_iou:.4f}. Model saved.")

    
    torch.save(model.cpu().state_dict(), 'waymo_last_model_state_dict.pth')
