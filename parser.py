...
from laserscan import SemLaserScan, SemLaserScanWaymo
import torchvision
...
from dswaymokitti import SegmentationDataset
...
class SemanticKitti(Dataset):
  ...
  def __getitem__(self, index):
    ...
    # return
    if self.concat:
      proj = F.pad(proj, (3, 3), value=0)
      proj_mask = F.pad(proj_mask, (3, 3), value=0)
      proj_labels = F.pad(proj_labels, (3, 3), value=0)
...
class Parser():
  ...
  def __init__(self,
               ...
               shuffle_train=True,
               iswaymo = False,
               concat = False ):  # shuffle training set?
    ...
    if self.concat:
      ds2 = SegmentationDataset(root = '/ari/users/ibaskaya/projeler/waymosalsa/data/waymo', split = 'training', 
      transform=None, pretransform=None, fastfill=None, iswaymo=True, width=2650, unknown=-1, xyz=True, concat=self.concat)
      self.train_dataset = ConcatDataset([self.train_dataset,ds2])
    ...
    if self.concat:
      self.valid_dataset = SegmentationDataset(root = '/ari/users/ibaskaya/projeler/waymosalsa/data/waymo', split = 'validation', 
      transform=None, pretransform=None, fastfill=None, iswaymo=True, width=2650, unknown=-1, xyz=True, concat=self.concat)
